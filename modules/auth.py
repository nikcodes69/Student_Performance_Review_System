...
def authenticate_with_google(google_email: str) -> dict:
    ...
    DELIBERATELY NOT ROLE-SCOPED, UNLIKE authenticate()'s expected_role:
    a Google account's role always comes from the database record it's
    linked to, never from which of the three login pages (Admin/Teacher/
    Student) the "Sign in with Google" button happened to be clicked on.
    Those pages are an entry point, not a claim to verify against --
    someone whose Google account is linked to a Teacher record is signed
    in as Teacher even if they clicked "Sign in with Google" on the
    Student page. This is intentional: the same real Google identity
    should reach the same real account and the same real dashboard
    regardless of which card on the landing page they happened to click.
